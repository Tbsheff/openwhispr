...
export function createMcpServer(db: Db, context: { authInfo?: AuthInfo; env?: Env } = {}): McpServer {
  ...
  const accessContext = accessContextFromAuth(context.authInfo);
  registerMemoryTools(server, db, accessContext);
  ...
  server.tool("get_note", "Get a note by ID with full content", {
    ...
    try {
      ...
      appendScopeFilters(filters, params, authorizeResourceScope({ workspace_id, team_id }, accessContext));
  ...
  server.tool("create_note", "Create a new note", {
    ...
    try {
      const scope = authorizeResourceScope({ workspace_id, team_id }, accessContext);
      const noteWorkspaceId = scope.workspaceId ?? null;
      const noteTeamId = scope.teamId ?? null;
  ...
  server.tool("delete_note", "Soft-delete a note", {
    ...
    try {
      ...
      if (r.rowCount === 0) return notFoundContent(`Note ${id} not found.`);
  ...
  server.tool("get_transcription", "Get a transcription by ID", {
    ...
    try {
      ...
      if (!row) return notFoundContent(`Transcription ${id} not found.`);
      return jsonContent(row);
    } catch (error) {
      return errorContent(error);
    }
  ...
  server.tool("create_transcription", "Store a new transcription", {
    ...
    try {
      const scope = authorizeResourceScope({ workspace_id, team_id }, accessContext);
      const row = await db.getOne(`INSERT INTO transcriptions (text, raw_text, provider, model, created_by, workspace_id, team_id) VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING id, text, provider, model, workspace_id, team_id, created_by, created_at`, [text, raw_text ?? null, provider ?? null, model ?? null, created_by ?? null, scope.workspaceId ?? null, scope.teamId ?? null]);
      return jsonContent(row);
    } catch (error) {
      return errorContent(error);
    }
  ...
  server.tool("create_folder", "Create a new folder", {
    ...
    try {
      ...
      const row = await db.getOne(`INSERT INTO folders (name, sort_order, workspace_id, team_id) VALUES ($1, $2, $3, $4) RETURNING id, name, is_default, sort_order, workspace_id, team_id, created_at`, [name, (max?.max ?? 0) + 1, scope.workspaceId ?? null, scope.teamId ?? null]);
      return jsonContent(row);
    } catch (error) {
      return errorContent(error);
    }
